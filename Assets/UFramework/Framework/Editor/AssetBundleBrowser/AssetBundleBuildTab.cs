...
namespace UnityEngine.AssetBundles
{
    ...
    public class AssetBundleBuildTab
    {
        ...
        public void OnEnable(Rect pos, EditorWindow parent)
        {
            ...
            m_MakeMD5 = new ToggleData(
               false,
               "Make MD5",
               "After build completes will make all files Md5",
               m_UserData.m_OnToggles);
        ...
        public void OnGUI(Rect pos)
        {
            ...
            using (new EditorGUI.DisabledScope (!AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory)) {
                ...
                newState = GUILayout.Toggle(
                    m_MakeMD5.state,
                    m_MakeMD5.content);
                if (newState != m_MakeMD5.state)
                {
                    if (newState)
                        m_UserData.m_OnToggles.Add(m_MakeMD5.content.text);
                    else
                        m_UserData.m_OnToggles.Remove(m_MakeMD5.content.text);
                    m_MakeMD5.state = newState;
                }
        ...
        private void ExecuteBuild()
        {
            ...
            if (m_MakeMD5.state)
                CreatMD5.CreatMD5XML(m_UserData.m_OutputPath);
